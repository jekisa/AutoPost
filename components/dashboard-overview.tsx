...
function PulseChange({ value }: { value: number | null }) {
  ...
  return <span className={`text-xs font-bold ${value >= 0 ? "text-emerald-600" : "text-rose-600"}`}>{value >= 0 ? "↑" : "↓"} {Math.abs(value).toFixed(1)}%</span>;
}
...
export function DashboardOverview() {
  ...
  return <div className="space-y-6">
    <section className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-[#F97362]">Your publishing cockpit</p><h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl dark:text-white">{greetingLabel(data.today)}, {data.greeting.name}!</h1></div><p className="text-sm font-semibold text-slate-500">{formatToWIB(data.today, "EEE, MMM dd yyyy")}</p></section>
    ...
    <section className="grid gap-3 md:grid-cols-3">
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center justify-between"><p className="text-xs font-black uppercase tracking-wide text-slate-500">Week Streak</p><TrendingUp className="text-[#F97362]" size={19} /></div><p className="mt-4 text-3xl font-black text-slate-950 dark:text-white">{data.summary.streak} minggu</p><p className="mt-2 text-sm font-bold text-violet-600">{data.summary.progressStatus}</p></div>
      <button type="button" onClick={() => { setGoalValue(data.summary.target ?? 3); setGoalOpen(true); }} className="rounded-3xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:border-violet-300 dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center justify-between"><p className="text-xs font-black uppercase tracking-wide text-slate-500">Posting Goals</p><Edit3 className="text-violet-600" size={17} /></div>{data.summary.target ? <><p className="mt-4 text-xl font-black text-slate-950 dark:text-white">{data.summary.currentWeekPosts} / {data.summary.target} posts this week</p><div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800"><div className="h-full rounded-full bg-gradient-to-r from-[#F97362] to-[#7C3AED]" style={{ width: `${Math.min(data.summary.currentWeekPosts / data.summary.target * 100, 100)}%` }} /></div><p className="mt-2 text-sm font-bold text-violet-600">{data.summary.progressStatus}</p></> : <><p className="mt-4 text-xl font-black text-slate-950 dark:text-white">No goals yet</p><span className="mt-3 inline-flex items-center gap-1 text-sm font-bold text-violet-600">Set Goal <ArrowUpRight size={15} /></span></>}</button>
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center justify-between"><p className="text-xs font-black uppercase tracking-wide text-slate-500">Comment Score</p><MessageCircle className="text-sky-600" size={19} /></div><p className="mt-4 text-3xl font-black text-slate-950 dark:text-white">{data.summary.commentAverage.toFixed(1)}</p><p className="mt-2 text-sm font-bold text-sky-600">{data.summary.commentScore}</p></div>
    </section>
